LPARA SmtpServer,SmtpFrom,SmtpTo,SmtpReply,jxSubject,jxBody,xAttachStr,jRetRecpt
*_Screen.Visible=.F. 
...
ELSE
  ...
  Kode = seeSendEmail(0,@SmtpTo,@EmptyStr,@EmptyStr,@jxSubject,@jxBody,@xAttachStr)
  if Kode < 0
    Temp = SPACE(128)
    Kode = seeErrorText(0,Kode,@Temp,128)
    ? Left(Temp,Kode)
